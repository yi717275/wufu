...
import { Edit, Save, X, Download, Plus, Trash2 } from 'lucide-react';
...
const MemberOrders: React.FC = () => {
  ...
  const userOrders = orders.filter(order => order.customerDetails.phone === user?.phone);
  ...
  const handleDownloadImage = async () => {
    if (orderRef.current === null || !selectedOrder) {
      console.error('Order reference is null or no order selected');
      return;
    }
